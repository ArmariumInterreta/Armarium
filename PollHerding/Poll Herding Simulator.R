...
#sampling_bias <- 0
avg_2month_change <- 1.5

require(foreach)
require(doParallel)

init_val <- round(rnorm(sims,mean=true_val,sd=avg_2month_change),2)
#init_val <- round(rnorm(sims,mean=-3,sd=sampling_err),2)
time_model <- sample.int(4,sims,replace=TRUE)
#time_model <- rep(4,sims)
...
colnames(W2_Polls_Ind) <- paste("P",1:pollsters,sep="")
...
simulateHerdedPolls <- function(init_sample,polling_avgs,absdev_limit,p_drawer){
  ...
  if(outliers > 0 && !is.na(outliers)){
    ...
    replacement[!is.na(replacement) & adj_vec < 0.75] <- (adj_sample[outlier_matrix][!is.na(replacement) & adj_vec < 0.75] - last_poll_avgs[outlier_matrix][!is.na(replacement) & adj_vec < 0.75]) * absdev_limit/abs(adj_sample[outlier_matrix][!is.na(replacement) & adj_vec < 0.75] - last_poll_avgs[outlier_matrix][!is.na(replacement) & adj_vec < 0.75])
    replacement[!is.na(replacement) & adj_vec >= 0.75] <- rnorm(sum(!is.na(replacement) & adj_vec >= 0.75),mean=(adj_sample[outlier_matrix][!is.na(replacement) & adj_vec >= 0.75] - last_poll_avgs[outlier_matrix][!is.na(replacement) & adj_vec >= 0.75])/2,sd=absdev_limit/4)
    replacement[!is.na(replacement)] <- replacement[!is.na(replacement)] + last_poll_avgs[outlier_matrix][!is.na(replacement)]
  ...
  if(outliers > 0 && !is.na(outliers)){
    ...
    adj_vec <- runif(outliers,0,1)
    adj_vec[(abs(adj_sample - last_poll_avgs)[outlier_matrix] >= 2*absdev_limit)] <- 0
    replacement[!is.na(replacement) & adj_vec < 0.65] <- (adj_sample[outlier_matrix][!is.na(replacement) & adj_vec < 0.65] - last_poll_avgs[outlier_matrix][!is.na(replacement) & adj_vec < 0.65]) * absdev_limit/abs(adj_sample[outlier_matrix][!is.na(replacement) & adj_vec < 0.65] - last_poll_avgs[outlier_matrix][!is.na(replacement) & adj_vec < 0.65])
    replacement[!is.na(replacement) & adj_vec >= 0.65] <- rnorm(sum(!is.na(replacement) & adj_vec >= 0.65),mean=(adj_sample[outlier_matrix][!is.na(replacement) & adj_vec >= 0.65] - last_poll_avgs[outlier_matrix][!is.na(replacement) & adj_vec >= 0.65])/2,sd=absdev_limit/4)
    replacement[!is.na(replacement)] <- replacement[!is.na(replacement)] + last_poll_avgs[outlier_matrix][!is.na(replacement)]
...
rm(list=c('cov_devs1','cov_devs1_herd','cov_devs2','cov_devs2_herd','cov_errs1','cov_errs1_herd','cov_errs2','cov_errs2_herd'))
...
hist(as.vector(matrix(as.matrix(Final_Polls_Ind),nrow=sims*pollsters,ncol=1)),border=NA,freq=FALSE,xlim=c(-6,6),ylim=c(0,0.5),col=rgb(1,0,0,0.25),breaks=25,main='Individual polls',xlab="Coalition 2pp in simulated polls")
...
hist(ind_avgs,freq=FALSE,border=NA,breaks=50,xlim=c(-6,6),ylim=c(0,0.75),col=rgb(1,0,0,0.25),main='Polling averages',xlab='Coalition 2pp in simulated polling average')
